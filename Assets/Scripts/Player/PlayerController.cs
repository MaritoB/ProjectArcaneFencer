...
public class PlayerController : MonoBehaviour, IDamageable
{
    ...
    public void HandleMovement( float movementSpeed)
    {
        ...
        Vector3 mouseDirection = GetMouseDirection(); // Necesitas devolver esta dirección desde tu método

        // 🔹 Calcular alineación
        float alignment = Vector3.Dot(movementVector.normalized, mouseDirection.normalized);

        // 🔹 Si se mueve hacia el mouse, aumentar velocidad
        float speedMultiplier = 1f;

        if (alignment > 0.7f) // Ajusta este valor (0.8 ≈ 36° de diferencia)
        {
            speedMultiplier = 1.25f; // Corre más rápido
        }
        if (alignment < -0.7f) // Ajusta este valor (0.8 ≈ 36° de diferencia)
        {
            speedMultiplier = 0.8f; // Corre más lento
        }
